...
public class TestAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    {
        return Context.Request.Headers["Authorization"] == "TestAdminScheme" ? HandleAuthenticateAsyncAdmin() : HandleAuthenticateAsyncUser();
    }
    ...
    private Task<AuthenticateResult> HandleAuthenticateAsyncUser()
    {
        ...
        var principal = new ClaimsPrincipal(identity);
        var ticket = new AuthenticationTicket(principal, "TestUserScheme");
        var result = AuthenticateResult.Success(ticket);
        return Task.FromResult(result);
    }
}